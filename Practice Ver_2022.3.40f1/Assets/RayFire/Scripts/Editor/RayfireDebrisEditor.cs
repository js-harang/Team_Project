...
namespace RayFireEditor
{
    ...
    public class RayfireDebrisEditor : Editor
    {
        ...
        void GUI_Emit()
        {
            GUICommon.Caption (TextDbr.gui_cap_event);
        ...
        void GUI_Main()
        {
            GUICommon.Caption (TextDbr.gui_cap_debris);
        ...
        void GUI_Properties()
        {
            GUICommon.Caption (TextDbr.gui_cap_props);
            GUI_Pool();
            GUI_Emission();
            GUI_Dynamic();
            GUI_Noise();
        ...
        void GUI_Collision()
        {
            ...
            if (fld_col == true)
            {
                ...
                int collidesWith = EditorGUILayout.MaskField (TextDbr.gui_col_mask, debris.collision.collidesWith, layerNames.ToArray());
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, TextDbr.gui_col_mask.text);
        ...
        void GUI_Rendering()
        {
            ...
            if (fld_rnd == true)
            {
                ...
                if (debris.rendering.t == true)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    string tag = EditorGUILayout.TagField (TextDbr.gui_ren_tag, debris.rendering.tag);
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObjects (targets, TextDbr.gui_ren_tag.text);
                        foreach (RayfireDebris scr in targets)
                        {
                ...
                if (debris.rendering.l == true)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    int layer = EditorGUILayout.LayerField (TextDbr.gui_ren_lay, debris.rendering.layer);
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObjects (targets, TextDbr.gui_ren_lay.text);
                        foreach (RayfireDebris scr in targets)
                        {
