...
namespace RayFireEditor
{
    ...
    public class RayfireRigidEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUICommon.Caption (TextRig.gui_cap_com);
            GUI_Fade();
            GUI_Reset();
        ...
        void GUI_Main()
        {
            GUICommon.Caption (TextRig.gui_cap_mn);
            GUICommon.Space ();
        ...
        void GUI_Simulation()
        {
            GUICommon.Caption (TextRig.gui_cap_sim);
            GUICommon.Space ();
        ...
        void GUI_Activation()
        {
            ...
            if (fld_act == true)
            {
                ...
                if (rigid.activation.l == true)
                {
                    ...
                    int lay = EditorGUILayout.LayerField (TextAcv.gui_act_lay, rigid.activation.lay);
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObjects (targets, TextAcv.gui_act_lay.text);
        ...
        void GUI_Limitations()
        {
            if (rigid.objTp == ObjectType.MeshRoot || rigid.dmlTp != DemolitionType.None)
            {
                ...
                if (fld_lim == true)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    string tag = EditorGUILayout.TagField (TextLim.gui_lim_tag, rigid.limitations.tag);
                    if (EditorGUI.EndChangeCheck())
                    {
        ...
        void GUI_Layer_Tag()
        {
            ...
            if (rigid.mshDemol.prp.l == false)
            {
                EditorGUI.BeginChangeCheck();
                int lay = EditorGUILayout.LayerField (TextMsh.gui_msh_adv_lay, rigid.mshDemol.prp.lay);
                if (EditorGUI.EndChangeCheck())
                {
                    Undo.RecordObjects (targets, TextMsh.gui_msh_adv_lay.text);
                    foreach (RayfireRigid scr in targets)
                    {
                        scr.mshDemol.prp.lay = lay;
                        SetDirty (scr);
                    }
                }
                GUICommon.Space ();
            ...
            if (rigid.mshDemol.prp.t == false)
            {
                EditorGUI.BeginChangeCheck();
                string tag = EditorGUILayout.TagField (TextMsh.gui_msh_adv_tag, rigid.mshDemol.prp.tag);
                if (EditorGUI.EndChangeCheck())
                {
                    Undo.RecordObjects (targets, TextMsh.gui_msh_adv_tag.text);
                    foreach (RayfireRigid scr in targets)
                    {
                        scr.mshDemol.prp.tag = tag;
                        SetDirty (scr);
                    }
                }
                GUICommon.Space ();
        ...
        void GUI_Info()
        {
            ...
            {
                if (rigid.physics.cc != null && rigid.physics.cc.Count > 0)
                {
                    if (rigid.clsDemol != null && rigid.clsDemol.cluster != null)
                    {
        ...
        void ClusterPreviewUI(RayfireRigid scr)
        {
            if (rigid.objTp == ObjectType.ConnectedCluster)
            {
                ...
                // Show nodes
                EditorGUI.BeginChangeCheck();
                scr.clsDemol.cn = GUILayout.Toggle (scr.clsDemol.cn, TextRig.gui_btn_conns, "Button", GUILayout.Height (22));
                scr.clsDemol.nd = GUILayout.Toggle (scr.clsDemol.nd, TextRig.gui_btn_nodes, "Button", GUILayout.Height (22));
                if (EditorGUI.EndChangeCheck())
                {
                    foreach (Object targ in targets)
                        if (targ as RayfireRigid != null)
                        {
                            (targ as RayfireRigid).clsDemol.cn = rigid.clsDemol.cn;
                            (targ as RayfireRigid).clsDemol.nd = rigid.clsDemol.nd;
                            SetDirty (targ as RayfireRigid); 
                        }
