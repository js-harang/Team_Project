...
namespace RayFireEditor
{
    ...
    public class RayfireDustEditor : Editor
    {
        ...
        void GUI_Emit()
        {
            GUICommon.Caption (TextDst.gui_cap_event);
        ...
        void GUI_Main()
        {
            GUICommon.Caption (TextDst.gui_cap_dust);
        ...
        void UI_Properties()
        {
            GUICommon.Caption (TextDst.gui_cap_props);
            GUI_Pool();
            GUI_Emission();
            GUI_Dynamic();
            GUI_Noise();
        ...
        void GUI_Collision()
        {
            ...
            if (fld_col == true)
            {
                ...
                int collidesWith = EditorGUILayout.MaskField (TextDst.gui_col_mask, dust.collision.collidesWith, layerNames.ToArray());
                if (EditorGUI.EndChangeCheck() == true)
                {
                    Undo.RecordObjects (targets, TextDst.gui_col_mask.text);
        ...
        void GUI_Rendering()
        {
            ...
            if (fld_rnd == true)
            {
                ...
                if (dust.rendering.t == true)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    string tag = EditorGUILayout.TagField (TextDst.gui_ren_tag, dust.rendering.tag);
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObjects (targets, TextDst.gui_ren_tag.text);
                        foreach (RayfireDust scr in targets)
                        {
                ...
                if (dust.rendering.l == true)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    int layer = EditorGUILayout.LayerField (TextDst.gui_ren_lay, dust.rendering.layer);
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObjects (targets, TextDst.gui_ren_lay.text);
                        foreach (RayfireDust scr in targets)
                        {
