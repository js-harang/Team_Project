...
namespace RayFire
{
    ...
    public class RayfireCombine : MonoBehaviour
    {
        ...
        bool SetData()
        {
            ...
            if (meshList.Count == 0)
            {
                RayfireMan.Debug("No meshes to combine");
                return false;
            }
            ...
            if (totalVerts >= 65535)
            {
                RayfireMan.Debug ("RayFire Combine: " + name + " combined mesh has more than 65535 vertices. UInt32 mesh Index Format will be used.", gameObject);
            }
