...
namespace RayFire
{
    ...
    public class RayfireDebris : MonoBehaviour
    {
        ...
        void SetReferenceMeshes(GameObject refs)
        {
            ...
            if (refs == null)
            {
                RayfireMan.Debug ("RayFire Debris: " + name + ": Debris reference not defined.", gameObject);
                return;
            }
            ...
            if (stMfs.Length == 0)
            {
                RayfireMan.Debug ("RayFire Debris: " + name + ": Debris reference mesh is not defined.", gameObject);
                return;
            }
            ...
            for (int i = 0; i < stMfs.Length; i++)
            {
                ...
                else RayfireMan.Debug ("RayFire Debris: " + stMfs[i].name + ": No mesh or amount of vertices too low.", gameObject);
            }
            
            // Check for deprecated burst property. Will be OBSOLETE
            if ((int)emission.burstType == 1)
            {
                RayfireMan.Debug ("RayFire Debris: " + name + ": Deprecated Burst Type property.", gameObject);
            }
