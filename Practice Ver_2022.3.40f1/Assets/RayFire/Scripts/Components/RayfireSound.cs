        ...
        void WarningCheck()
        {
            ...
            if (initialization.enable == false &&
                ...
                if (rigid == null && rigidRoot == null)
                    RayfireMan.Debug ("RayFire Sound: " + name + " Warning. Sound component has no attached Rigid or RigidRoot component", gameObject);
            ...
            if (initialization.enable == false &&
                ...
                RayfireMan.Debug ("RayFire Sound: " + name + " Warning. All events disabled", gameObject);
            
            // No clips
            if (initialization.enable == true)
                if (initialization.clip == null && initialization.HasClips == false)
                    RayfireMan.Debug ("RayFire Sound: " + name + " Warning. Initialization sound has no clips to play", gameObject);
            if (activation.enable == true)
                if (activation.clip == null && activation.HasClips == false)
                    RayfireMan.Debug ("RayFire Sound: " + name + " Warning. Activation sound has no clips to play", gameObject);
            if (demolition.enable == true)
                if (demolition.clip == null && demolition.HasClips == false)
                    RayfireMan.Debug ("RayFire Sound: " + name + " Warning. Demolition sound has no clips to play", gameObject);
            if (collision.enable == true)
                if (collision.clip == null && collision.HasClips == false)
                    RayfireMan.Debug ("RayFire Sound: " + name + " Warning. Collision sound has no clips to play", gameObject);
        }
