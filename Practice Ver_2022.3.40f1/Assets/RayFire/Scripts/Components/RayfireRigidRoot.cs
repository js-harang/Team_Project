...
namespace RayFire
{
    ...
    public class RayfireRigidRoot : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            {
                RayfireMan.Debug (strRoot + name + " has no children. RigidRoot should be used on object with children.", gameObject);
                return;
            }
        ...
        void InitConnectivity()
        {
            ...
            if (activation.cnt != null)
            {
                ...
                if (RayfireMan.debugStatic == true)
                    if (cached == true && activation.cnt.cluster.cachedHost == false)
                        RayfireMan.Debug (strRoot + name + " object has Editor Setup but its connection data is not cached. Reset Setup and use Editor Setup again.", gameObject);
            ...
            if (RayfireMan.debugStatic == true)
            {
                if (activation.con == true && activation.cnt == null)
                    RayfireMan.Debug (strRoot + name + " object has enabled Connectivity activation but has no Connectivity component.", gameObject);
                if (activation.con == false && activation.cnt != null)
                    RayfireMan.Debug (strRoot + name + " object has Connectivity component but activation by Connectivity is disabled.", gameObject);
            }
        }
        ...
        */
        ...
        void NullCheck()
        {
            ...
            {
                RayfireMan.Debug (strRoot + name + " has missing shards. Reset Setup and use Editor Setup again.", gameObject);
                ResetSetup();
            }
            ...
            {
                RayfireMan.Debug (strRoot + name + " has missing Rigid component with MeshRoot object type. Reset Setup and use Editor Setup again.", gameObject);
                ResetSetup();
            }
        ...
        static bool ShardFilter(RFShard shard, RayfireRigidRoot scr)
        {
            ...
            if (shard.mf == null)
            {
                RayfireMan.Debug (strRoot + shard.tm.name + " has no MeshFilter. Shard won't be simulated.", shard.tm.gameObject);
            ...
            {
                RayfireMan.Debug (strRoot + shard.tm.name + " has no mesh. Shard won't be simulated.", shard.tm.gameObject);
                scr.destroyShards.Add (shard);
                return false;
            }
            ...
            {
                RayfireMan.Debug (strRoot + shard.tm.name + " has 3 or less vertices. Shard can't get Mesh Collider and won't be simulated.", shard.tm.gameObject);
                scr.destroyShards.Add (shard);
                return false;
            }
            ...
            if (RayfireMan.colliderSizeStatic > 0)
            {
                ...
                {
                    RayfireMan.Debug (strRoot + shard.tm.name + " is very small and won't be simulated.", shard.tm.gameObject);
                    scr.destroyShards.Add (shard);
                    return false;
                }
            }
            ...
            if (scr.physics.pc == true && shard.mf.sharedMesh.vertexCount < RayfireMan.coplanarVertLimit)
            {
                ...
                {
                    RayfireMan.Debug (strRoot + shard.tm.name + " has planar low poly mesh. Shard can't get Mesh Collider and won't be simulated.", shard.tm.gameObject);
                    scr.destroyShards.Add (shard);
                    return false;
                }
            }
