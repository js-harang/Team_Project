...
namespace RayFire
{
    ...
    public class RayfireRigid : MonoBehaviour
    {
        ...
        void InitConnectivity()
        {
            ...
            if (RayfireMan.debugStatic == true)
                if (activation.con == true && activation.cnt == null)
                    RayfireMan.Debug ("RayFireRigid: " + name + " object has enabled Connectivity activation but has no Connectivity component.", gameObject);
        }
        ...
        public void Slice()
        {
            ...
            if (HasSlices == false)
            {
                RayfireMan.Debug (RFLimitations.rigidStr + name + " has no defined slicing planes.", gameObject);
                return;
            }
