    ...
    public class RFReferenceDemolition
    {
        ...
        public static bool DemolishReference (RayfireRigid scr)
        {
            if (scr.dmlTp == DemolitionType.ReferenceDemolition)
            {
                ...
                {
                    RayfireMan.Debug (RFLimitations.rigidStr + scr.name + " Demolition Reference object has already initialized Rigid. Set By Method Initialization type or Deactivate reference.", scr.gameObject);
                    return true;
                }
