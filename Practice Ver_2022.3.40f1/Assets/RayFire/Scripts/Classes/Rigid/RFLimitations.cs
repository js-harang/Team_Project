...
namespace RayFire
{
    ...
    public class RFLimitations
    {
        ...
        public IEnumerator DemolishableCor(RayfireRigid scr)
        {
            ...
            while (scr.dmlTp != DemolitionType.None)
            {
                // Max depth reached
        ...
        public static void MeshRootCheck (RayfireRigid scr)
        {
            ...
            if (scr.HasFragments == true)
            {
                for (int f = 0; f < scr.fragments.Count; f++)
                {
                    ...
                    {
                        RayfireMan.Debug (rigidStr + scr.name + " object has missing fragments. Reset Setup and used Editor Setup again", scr.gameObject);
        ...
        public static void Checks (RayfireRigid scr)
        {
            ...
            if (scr.simTp == SimType.Static)
            {
                ...
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Simulation Type set to " + scr.simTp.ToString() + " but Demolition Type is not None. Static object can not be demolished. Demolition Type set to None.", scr.gameObject);
                    scr.dmlTp = DemolitionType.None;
                }
            }
            ...
            if (scr.simTp != SimType.Static)
            {
                ...
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Simulation Type set to " + scr.simTp.ToString() + " but object is Static. Turn off Static checkbox in Inspector.", scr.gameObject);
                }
            }
            ...
            if (scr.objTp == ObjectType.Mesh)
            {
                ...
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Object Type set to " + scr.objTp.ToString() + " but object has no mesh. Object Excluded from simulation.", scr.gameObject);
                ...
                if (scr.mshDemol.cnv == RFDemolitionMesh.ConvertType.Connectivity)
                {
                    ...
                    {
                        RayfireMan.Debug (rigidStr + scr.name + " Convert property set to " + scr.mshDemol.cnv.ToString() + " but Fragments Sim Type is not Inactive or Kinematik. Convertation disabled.", scr.gameObject);
                ...
                if (scr.dmlTp != DemolitionType.None && scr.dmlTp != DemolitionType.ReferenceDemolition)
                {
                    ...
                    {
                        RayfireMan.Debug (rigidStr + scr.name + " Mesh is not readable. Demolition type set to None. Open Import Settings and turn On Read/Write Enabled property", scr.mFlt.gameObject);
                        scr.dmlTp         = DemolitionType.None;
            ...
            else if (scr.objTp == ObjectType.NestedCluster || scr.objTp == ObjectType.ConnectedCluster)
            {
                ...
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Object Type set to " + scr.objTp.ToString() + " but object has no children. Object Excluded from simulation.", scr.gameObject);
                    scr.physics.exclude = true;
                }
            }
            ...
            {
                if (scr.skr == null)
                    RayfireMan.Debug (rigidStr + scr.name + " Object Type set to " + scr.objTp.ToString() + " but object has no SkinnedMeshRenderer. Object Excluded from simulation.", scr.gameObject);
            ...
            if (scr.dmlTp != DemolitionType.None)
            {
                    ...
                    scr.objTp == ObjectType.ConnectedCluster)
                {
                    ...
                    {
                        RayfireMan.Debug (rigidStr + scr.name + " Object Type set to " + scr.objTp.ToString() + " but Demolition Type is " + scr.dmlTp.ToString() + ". Demolition Type set to Runtime.", scr.gameObject);
                    ...
                    scr.dmlTp == DemolitionType.AwakePrefragment)
                {
                    ...
                    {
                        RayfireMan.Debug (rigidStr + scr.name + "Demolition Type is " + scr.dmlTp.ToString() + ". Has no Shatter component, but Use Shatter property is On. Use Shatter property was turned Off.", scr.gameObject);
            ...
            if (scr.dmlTp == DemolitionType.None)
            {
                ...
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Demolition Type set to None. Had precached meshes which were destroyed.", scr.gameObject);
                    scr.DeleteCache();
                }

                if (scr.objTp == ObjectType.Mesh && scr.HasFragments == true)
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Demolition Type set to None. Had prefragmented objects which were destroyed.", scr.gameObject);
                    scr.DeleteFragments();
                }
            }
            ...
            else if (scr.dmlTp == DemolitionType.Runtime)
            {
                if (scr.HasMeshes == true)
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Demolition Type set to Runtime. Had precached meshes which were destroyed.", scr.gameObject);
                    scr.DeleteCache();
                }

                if (scr.objTp == ObjectType.Mesh && scr.HasFragments == true)
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Demolition Type set to Runtime. Had prefragmented objects which were destroyed.", scr.gameObject);
                    scr.DeleteFragments();
                }
                ...
                if (scr.mshDemol.use == true && scr.mshDemol.ch.tp != CachingType.Disable)
                {
                    ...
                    {
                        RayfireMan.Debug (rigidStr + scr.name + " Demolition Type is Runtime, Use Shatter is On. Unsupported fragments type. Runtime Caching supports only Voronoi, Splinters, Slabs and Radial fragmentation types. Runtime Caching was Disabled.", scr.gameObject);
            ...
            {
                if (scr.HasMeshes == true)
                    RayfireMan.Debug (rigidStr + scr.name + " Demolition Type set to Awake Precache. Had manually precached Unity meshes which were overwritten.", scr.gameObject);
                
                if (scr.HasFragments == true)
                {
                    RayfireMan.Debug (rigidStr + scr.name + " Demolition Type set to Awake Precache. Had manually prefragmented objects which were destroyed.", scr.gameObject);
                    scr.DeleteFragments();
                }
            }
            ...
            else if (scr.dmlTp == DemolitionType.AwakePrefragment)
            {
                ...
                {
                    if (scr.HasFragments == true)
                        RayfireMan.Debug (rigidStr + scr.name + " Demolition Type set to Awake Prefragment. Has manually prefragmented objects", scr.gameObject);
                    if (scr.HasMeshes == true)
                        RayfireMan.Debug (rigidStr + scr.name + " Demolition Type set to Awake Prefragment. Has manually precached Unity meshes.", scr.gameObject);
                }
            }
