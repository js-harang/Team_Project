...
namespace RayFire
{
    ...
    public class RFDemolitionMesh
    {
        ...
        public static bool SetupRuntimeConnectedCluster (RayfireRigid rigid, bool awake)
        {
            ...
            if (awake == true)
            {
                // Destroy fragments rigid, rigidbody, particles
                for (int i = rigid.fragments.Count - 1; i >= 0; i--)
                {
        ...
        public static void SetupRuntimeConnectivity(RayfireRigid scr, bool awake)
        {
            ...
            {
                RayfireMan.Debug (RFLimitations.rigidStr + scr.name + " Convert property set to " + scr.mshDemol.cnv.ToString() + " but object has no Connectivity component. Convertation disabled.", scr.gameObject);
